...
from killtask import kill_chrome_processes
...
credentials_file = 'work_order.json'
...
service = Service(ChromeDriverManager("144.0.7559.61").install())
driver = webdriver.Chrome(service=service, options=chrome_options)
driver.maximize_window()
driver.execute_cdp_cmd("Page.bringToFront", {})

FuseLogin = fuse_login(driver, credentials, log_file)
# base_folder  = r"C:\Users\Administrator\OneDrive - ITG Communications, LLC\Work Order Import\comcast\Output"
base_folder = r"C:\Users\Administrator\Documents\Work Order Import\comcast\Output"
...
def process_raw_file(folder_path, raw_excel, date_folder, extaction_file, department, log_file):
    print("PROCESSING RAW FILE:", raw_excel)
    print("STEP 1: Checking & inserting missing columns from MAIN\n")
    area_column, rf_column, upload_direct = insert_columns_from_main(folder_path, raw_excel, department, log_file)
    if upload_direct:
    ...
    if department not in key_to_lookup:
        ...
        log_message(log_file, f"No lookup mapping found for department '{department}'\n")
        return
    ...
    for row in range(2, raw_ws.max_row + 1):
        ...
        rte_values.append(value_str)

    print(f"Total RteC values found: {len(rte_values)}")
    log_message(log_file, f"Total RteC values found: {len(rte_values)}")
    ...
    if not matched_rows:
        ...
        log_message(log_file,"No matching RteC values.\n")
        return
...
for dept in departments:
    ...
    log_message(log_file,f"Checking Department: {dept}")
    if dept in dept_status and dept_status[dept].lower() == "no job found":  
        ...
        log_message(log_file,f"Skipping department '{dept}' because Status No Job Found")
        recipient_emails = credentials['sdavis']
        cc_emails = credentials['ybotID']
        subject = f" Department {dept} Skipped due to No Job Found"
        body_message = f"The department '{dept}' has been skipped as the status returned was No Job Found."
        body_message1 = ""
        attachment_path = ""
        try:
            Sendmail(recipient_emails, cc_emails, subject, body_message, body_message1, attachment_path)
    ...
    if not os.path.isdir(dept_folder):
        ...
        recipient_emails = credentials['ybotID']
        cc_emails = credentials['ybotID']
        subject = f" {dept} Department folder not found"
        body_message = f"{dept} Department folder not found.Kindly check the issue."
        body_message1 = ""
        attachment_path = ""
        try:
            Sendmail(recipient_emails, cc_emails, subject, body_message, body_message1, attachment_path)
    ...
    if not os.path.isdir(date_folder):
        ...
        log_message(log_file,f"Date folder not found: {date_folder}")
        continue
    ...
    if not excel_files:
        ...
        log_message(log_file,"No Excel file found in date folder")
        recipient_emails = credentials['ybotID']
        cc_emails = credentials['ybotID']
        subject = f" {dept} Excel file not found"
        body_message = f"{dept} Department Excel file not found."
        body_message1 = ""
        attachment_path = ""
        try:
            Sendmail(recipient_emails, cc_emails, subject, body_message, body_message1, attachment_path)
    ...
    for excel_name in excel_files:
        ...
        log_message(log_file,f"Processing file: {excel_path}")

        process_raw_file(folder_path, excel_path, date_folder, extaction_file, dept, log_file)



def kill_excel_background():
    if sys.platform != "win32":
        return False
    try:
        result = subprocess.run(["taskkill", "/F", "/IM", "EXCEL.exe", "/T"],capture_output=True,text=True,timeout=10)
        if result.returncode == 0:
            print("All Excel processes killed successfully (including background)")
            return True
        elif "not found" in result.stderr.lower() or result.returncode == 128:
            print("No Excel processes found running")
            return True
        else:
            print(f"Taskkill returned error: {result.stderr}")
            return False
    except subprocess.TimeoutExpired:
        print("Timeout while trying to kill Excel")
        return False
    except Exception as e:
        print(f"Error killing Excel: {e}")
        return False
kill_excel_background()
try:
    driver.execute_cdp_cmd("Page.bringToFront", {})
    driver.execute_script("""alert("Work Order Import Completed Successfully for ALL Departments!");""")
    time.sleep(5)
